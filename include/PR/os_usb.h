...
typedef struct OSBbUsbInfo_s {
    /* 0x0000 */ u8 ua_type;            /* controller mode (host or device) */
    /* 0x0001 */ u8 ua_class;           /* device class */
    /* 0x0002 */ u8 ua_subclass;        /* subclass */
    /* 0x0003 */ u8 ua_protocol;        /* subclass */
    /* 0x0004 */ u16 ua_vendor;         /* vendor */
    /* 0x0006 */ u16 ua_product;        /* product */
    /* 0x0008 */ u8 ua_cfg;             /* configuration */
    /* 0x0009 */ u8 ua_ifc;             /* interface */
    /* 0x000A */ u8 ua_ep;              /* endpoint */
    /* 0x000B */ u8 ua_speed;           /* LS/FS/HS */
    /* 0x000C */ u8 ua_mode;            /* R, W or R/W */
    /* 0x000D */ u8 ua_blksize;         /* block size */
    /* 0x000E */ u8 ua_support;         /* have driver */
    /* 0x000F */ u8 ua_state;           /* device state (attached?) */
    /* 0x0010 */ u8* ua_mfr_str;        /* manufacturer string */
    /* 0x0014 */ u8* ua_prod_str;       /* product description string */
    /* 0x0018 */ u8* ua_driver_name;    /* driver name */
} OSBbUsbInfo; // size = 0x1C
...
extern s32 osBbUsbSetCtlrModes(s32 which, u32 mask);
extern u32 osBbUsbGetResetCount(s32 which);
extern s32 osBbUsbDevQuery(s32 which, OSBbUsbInfo *ip, s32 nip);
extern s32 osBbUsbDevGetHandle(s32 which, OSBbUsbInfo *ip, OSBbUsbHandle *hp);
extern s32 osBbUsbDevFreeHandle(OSBbUsbHandle h);
extern s32 osBbUsbDevRead(OSBbUsbHandle h, u8 *buf, s32 len, u64 off);
extern s32 osBbUsbDevWrite(OSBbUsbHandle h, u8 *buf, s32 len, u64 off);
